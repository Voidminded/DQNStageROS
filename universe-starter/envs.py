...
def create_env(env_id, client_id, remotes, **kwargs):
    return create_stage_env(env_id, client_id, remotes, **kwargs)
...
def create_stage_env(env_id, client_id, remotes, **_):
    env = gym.make(env_id)
    env.initRos( client_id)
    env = Vectorize(env)
    env = StageRescale(env)
    env = EpisodeID(env)
...
def _process_stage_frame(frame):
    frame = cv2.resize(frame[0], (42, 42))
